...
const { Stocks, TempStocks, LastStocks } = require('./model')
...
async function insert(list, type) {
    try {
        ...
        if (type === 'temp') {
            await mongoose.connection.db.dropCollection('tempstocks')
        }else if(type === 'history') {
            await mongoose.connection.db.dropCollection('laststocks')
        }
